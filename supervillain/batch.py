...
def resolve_batch_cls(tag):
    ...
    if not tag:
        return None
    if tag == 'Form':
        from supervillain.compact.compact import Form
        return Form
...
class Batch:
    ...
    ndarray slice, or ``cls``-wrapped element such as :class:`~supervillain.compact.Form`).
    ...
    cls:
        Optional element class (:class:`~supervillain.compact.Form`, etc.).
    ...
    item_kwargs:
        ...
        (e.g. ``degree``, ``lattice`` for :class:`~supervillain.compact.Form`).
    '''
