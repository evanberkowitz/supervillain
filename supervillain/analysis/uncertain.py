...
A common notation for symmetric errors, explained on `Wikipedia`_ and by `NIST`_,
...
The :class:`~.Uncertain` class takes a mean and uncertainty and helps produce this formatted
shorthand so that
...
We can use `scientific E notation`_ by passing ``e`` in the format string.
...
.. _Wikipedia: https://en.wikipedia.org/wiki/Uncertainty#In_measurements
.. _NIST: https://physics.nist.gov/cgi-bin/cuu/Info/Constants/definitions.html
.. _scientific E notation: https://en.wikipedia.org/wiki/Scientific_notation#E_notation
